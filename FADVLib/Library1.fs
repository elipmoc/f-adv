...
let private usingList=
    SyntaxList[|
        ...
        SyntaxFactory.UsingDirective(SyntaxFactory.ParseName "FADVLib.Api")
    |]
        ...
        SyntaxFactory.Block [|
            SyntaxFactory.ExpressionStatement(
                SyntaxFactory.InvocationExpression(
                    SyntaxFactory.ParseExpression "GameForm.startApplication",
                    SyntaxFactory.ArgumentList(
                        SyntaxFactory.SeparatedList[| 
                            SyntaxFactory.Argument(SyntaxFactory.ParseExpression "0")
                        |]
                    )
                )
            )
...
let private addAssembly :MetadataReference[]=
    [|
        ...
        MetadataReference.CreateFromFile  typeof<Api.GameForm.GameForm>.Assembly.Location :>MetadataReference
    |]
...
let private emit (fileName:string) (compilation:CSharpCompilation)  : seq<Diagnostic> =
    let option=compilation.Options.WithOutputKind OutputKind.WindowsApplication
    compilation.WithOptions(option)
        .Emit(fileName).Diagnostics
    :>seq<Diagnostic>
